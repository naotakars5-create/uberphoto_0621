...
## 動作確認（一気通貫）
1. タブB `/customer` でプラン選択 → 名前入力 → **撮影スポットを地図のピン or プルダウンから選択**（記入不要・きれいに撮れる時間帯のヒント付き） → **人数・シーン・要望メモ**を任意入力 → 「決済して撮影を依頼」（スタブ即成功）
2. 近くのカメラマンから選ぶ（**おまかせ・最短**／指名／カードをタップでプロフィール＆作例＆レビュー）
3. 「向かっています」地図＋ETA画面 → 右下の**チャットボタンでカメラマンと連絡**（合流のやりとり）→（デモカメラマンは数秒で写真が自動到着＋ひとこと返信）
4. 「写真を見る」 → ギャラリーで閲覧・**ハートでお気に入り → お気に入りだけ保存**・かんたん編集・全保存
5. 撮影完了後、**カメラマンを星＋コメントで評価**（実際に保存され、プロフィールのレビューに反映）
6. 本物のカメラマン役は別タブ `/photographer` で待機オン → **「プロフィールを編集」で表示名・自己紹介・得意ジャンル・プロフィール写真・作例をアップロード**（お客様の選択画面に即反映）→ 選ばれると**撮影ブリーフ（場所・人数・シーン・要望）**を確認 → **お客様とチャットでやりとり** → 写真アップロード → 完了。受け取ったレビューは待機画面に表示
7. `/operator` で件数・状態がリアルタイム更新。トップ画面は**いまの待機人数をリアルタイム表示**
