...
## 動作確認（一気通貫）
1. タブB `/customer` でプラン選択 → 名前入力 → **撮影スポットをワンタップ選択**（きれいに撮れる時間帯のヒント付き） → **人数・シーン・要望メモ**を任意入力 → 「決済して撮影を依頼」（スタブ即成功）
...
6. 本物のカメラマン役は別タブ `/photographer` で待機オン → **「プロフィールを編集」で表示名・自己紹介・得意ジャンル・プロフィール写真・作例をアップロード**（お客様の選択画面に即反映）→ 選ばれると**撮影ブリーフ（場所・人数・シーン・要望）**を確認して撮影 → 写真アップロード → 完了。受け取ったレビューは待機画面に表示
7. `/operator` で件数・状態がリアルタイム更新
